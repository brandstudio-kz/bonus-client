...
abstract class Order extends Model
{
    ...
    public static function boot()
    {
        ...
        static::updating(function($order) {
            $old = $order->getOriginal(static::bonusesStatusField());
            if ($order->bonusesCheck()) {
                if (!static::bonusesIsSuccess($old) && static::bonusesIsSuccess($order->{static::bonusesStatusField()})) {
                ...
                } else if (static::bonusesIsSuccess($old) && !static::bonusesIsSuccess($order->{static::bonusesStatusField()})) {
                    Bonus::cancelBonus([
                        'bonus' => [
                            'order_id' => $order->id,
                        ],
